...
async fn main() -> anyhow::Result<()> {
    ...
    for (query, expected_keywords) in &queries {
        ...
        let results = store
            .search(&query_emb, &anima_embed::SparseVector::default(), query, &ns, &SearchMode::Vector, 200, &large_config, &[("content".to_string(), 1.0)])
            .await?;
        ...
        for &nbr_limit in &neighbor_limits {
            ...
            for (query, direct_kws, expanded_kws) in &graph_queries {
                ...
                let search_results = store
                    .search(&query_emb, &anima_embed::SparseVector::default(), query, &ns, &SearchMode::Hybrid, search_top_k, &large_config, &[("content".to_string(), 1.0)])
                    .await?;
